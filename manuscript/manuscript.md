...
sk2bGrow makes PTR estimation practical at sequencing depths where current sketch-based coverage gates commonly return no estimate. The controlled benchmark attributes the shallow-depth gain primarily to coordinate-aware V-fitting rather than to landmark determinism: 2bRAD anchors and density-matched FracMinHash landmarks were not significantly different after density matching. The practical contribution of 2bRAD is therefore not universal accuracy superiority, but a wet-lab-realizable landmark panel with cross-enzyme QC strata and fusion redundancy. sk2bGrow estimates remain bounded by slope compression at very low depth, GC/reference-quality limits, and strain-complexity effects. Open implementation, provenance-tracked benchmarks, and explicit negative controls support its use as a shallow-depth growth-inference module that can be combined with longitudinal microbiome and disease-prediction analyses.
